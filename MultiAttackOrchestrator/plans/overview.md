...
seam is backed by an **in-memory mock**. In Part 2 the same seam is backed by a **TCP client**
...
- **The device is the authority on outcomes.** A stage doesn't roll dice locally; it asks the
  connection to run the step, and the connection (mock or real) reports success/failure or drops.
...
In Part 1 the in-memory mock plays the device's role and is **scriptable**, so tests can force
...
│   │   ├── mock.py                 # InMemoryDeviceConnection (Part 1 stand-in)
...
│   └── demo.py                     # small runnable example against the mock (optional)
└── tests/
    ├── __init__.py
    ├── conftest.py                 # shared fixtures (scripted mocks, sample devices)
    ├── test_models.py              # phase 1
    ├── test_connection.py          # phase 2 (mock + session + reconnect)
    ├── test_resolver.py            # phase 3
    ├── test_execution.py           # phase 4 (retry / crash-restart / drop)
    ├── test_extraction.py          # phase 5 (each mode + partial)
    └── test_orchestrator.py        # phase 6 (end-to-end against the mock)
```
...
| 2 | `phase2-connection.md` | `DeviceConnection` protocol + error types, in-memory mock, provider, `DeviceSession` | 1 |
...
Each phase ships with its own unit tests against the scriptable mock. Integration tests that run
...
- **Deterministic by construction.** The mock is scripted (or seeded), so no test depends on
  luck. Probability drives *ranking* (pure arithmetic, trivially testable) and, when we want it,
  the mock's own seeded outcomes — never an unseeded coin flip inside a test.
- **Each seam tested in isolation** (reqs matching, resolver ordering, extractor modes) plus
  **end-to-end** flows through `MultiAttackOrchestrator` against the mock.
